...
def fully_connected(enco,drop_param):
    flat = Flatten()(enco)
    # After experimentation, 2 dropout layers are better than one
...
# Ask the user to choose the hyperparameters for each experiment
...
while (repeat == True):
  ...
  # Get the encoder layers only from the loaded autoencoder model
  outputs = [layer.output for layer in load_model.layers]
  encoded_output = outputs[int(total_layers)-1]

  input_img = keras.Input(shape=(28,28,1))
  
  # Use the encoder layers(including the input layer = outputs[0]), to construct a fully connected layer
  ...
  # Do NOT train the encoder part again. It was already done in autoencoder.py
...
# Print 9 correct images as a sample (it would be time consuming to print all the correct images)
